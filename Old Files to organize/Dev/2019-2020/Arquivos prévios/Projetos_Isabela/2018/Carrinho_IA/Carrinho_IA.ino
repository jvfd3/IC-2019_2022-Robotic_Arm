  ...
  int TempoGirar = 1; //esse é o tempo para o carrinho girar em 45º com uma bateria de 9v.
  int distanciaObstaculo = 30; //distância para o carrinho parar e recalcular o melhor caminho
  int velocidadeMotores = 80; // velocidade ideal para que os motores funcionem com uma bateria 9v.
  ...
  void setup(){    
    ...
    motor1.setSpeed(velocidadeMotores);     // Define a velocidade para os motores
  ...
  // Função para chamar outras funções e definir o que o carrinho fará  
  void pensar(){    
   ...
   if (distancia > distanciaObstaculo) {  // Se a distância for maior que 30 cm  
     rotacao_Frente(); //carrinho anda para frente   
   }else{   
     rotacao_Parado();  //para o carrinho  
  ...
  char calculaMelhorDistancia(){    
   ...
   if (maiorDistancia <= distanciaObstaculo) { //distância limite para parar o carrinho   
     rotacao_Re();    
  ...
  // Função para deixar o sensor "olho" do carrinho no centro    
  ...
  {    
   motor1.run(BACKWARD);    //o carrinho dá uma ré para não colidir ao girar 
   motor2.run(BACKWARD);      
   delay(50);  
