...
function BluetoothList() {
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        setBolEnable(enable);
        console.log
        first_address = list[0].address
        setAddress(first_address);
      } catch (error) {
        console.log('erro em useEffect' + error)
      }
    }
  ...
  const renderEmpty = () => {
    return (
      <View style={styles.container6}>
        <Image
          style   = {styles.icon}
          source  = {require('./MiscJV/Icons/empty.png')}
        />
  ...
  const renderItem = ({item}) => {
    return (
      <Device
        {...item}
        iconLeft  ={require('./MiscJV/Icons/devices.png'  )}
        iconRight ={require('./MiscJV/Icons/cog.png'      )}
        onPress   ={() => {connect()}}
      />
    )
  }
