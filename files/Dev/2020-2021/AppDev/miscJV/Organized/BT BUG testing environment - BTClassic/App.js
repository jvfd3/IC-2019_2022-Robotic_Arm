...
const App = () => {
  ...
  useEffect(() => {
    async function init() {
      try {
        var   isBTEnabledVar=await BTClassic.isBluetoothEnabled(); 
        setisBTEnabled(isBTEnabledVar);

        if(isBTEnabledVar){
          const listUseEffect = await BTClassic.getBondedDevices();
  ...
  async function connect(DeviceAddress){
    try {
      ...
      await BTClassic.connectToDevice(DeviceAddress);

      setIsConnected(await BTClassic.isDeviceConnected(DeviceAddress));
      
     
      if (isConnected) {
        // await BluetoothSerial.disconnect(DeviceAddress);
        await BTClassic.disconnectFromDevice(DeviceAddress);
        setAddress('');
  ...
  async function sendDataLength(info){
    try {
      ...
      await BTClassic.writeToDevice(encodedMsg, address);
      console.log('sent\n');
      var data=255-lengb;
      console.log('\n192\n',lengb,'\n0\n0\n',data,'\n192\n');
    ...
    async function justSend(info){
      try {
        console.log('vai>');
        // await BTClassic.writeToDevice(address,'1234567','hex');      //ok '3'
        // await BTClassic.writeToDevice(address,'1234567','binary');   //ok '7'
        // await BTClassic.writeToDevice(address,info);                 //ok '7'
        // await BTClassic.writeToDevice(address,'1234567','base64');   //ok '7'
        // await BTClassic.writeToDevice(address,'1234567','utf-8');    //ok '7'
        // await BTClassic.writeToDevice(address,'1234567','ascii');    //ok '7'
        // await BTClassic.writeToDevice(address,'1234567','utf8');     //ok '7'
        // await BTClassic.writeToDevice(address,'1234567','latin1');   //ok '7'
        // await BTClassic.writeToDevice(address,'1234567','utf16le');  //ok '8'
        // await BTClassic.writeToDevice(address,'1234567','ucs2');     //ok '14'
        // await BTClassic.writeToDevice(address,'1234567','ucs-2');    //ok '14'

        
        await BTClassic.writeToDevice(address,'256');                 //ok '7'
        console.log('<foi');
      } catch (e) {
        console.log('justSend Error {',e,'} justSend Error ');
      }
    }
  ...
  async function enableBluetooth(){
    try {
      ...
      if(!isBTEnabled){
        await BTClassic.requestBluetoothEnabled();
        setList(await BTClassic.startDiscovery());
      }
      await BTClassic.cancelDiscovery();
      setisBTEnabled(true);
    } catch (e) {
  ...
  async function disableBluetooth(){
    try {
      ...
      await BTClassic.cancelDiscovery();
      // await BTClassic.disable(); //MAY NOT EXIST
    ...
    return (
      <View style={[styles.view3,styles.center,{flexDirection:'row'}]}>
        <View style={[styles.view4,styles.square,styles.center,{flex:0, width:66}]}  >
          <Image style={[styles.square,{height:55}]} source={require('./miscJV/app/icons/empty.png')} />
        </View>
  ...
  const renderItem = ({ item }) => {
    return (
      ...
      >
        <View style={[styles.view4,styles.square,styles.center,{flex:0, width:66}]}  >
          <Image style={[styles.square,{height:55}]} source={require('./miscJV/app/icons/devices.png')} />
        </View>
        ...
        <View style={[styles.view4,styles.square,styles.center,{flex:0, height:66,}]}  >
          <Image style={[styles.square,{height:55}]} source={require('./miscJV/app/icons/cog.png')} />
        </View>
      </TouchableOpacity>
    )
  }
